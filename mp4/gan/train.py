...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250, 
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            # Zero gradients in the discriminator
            D_solver.zero_grad()
            ...
            # Compute the discriminator output for real and fake images
            #print(real_images.shape)
            D_real = D(real_images)

            #fake_images = fake_images.reshape((batch_size, 1,real_images.shape[2], real_images.shape[3] ))
            
            #print(fake_images.shape)
            D_fake = D(fake_images)# Compute the discriminator loss
            d_error = discriminator_loss(D_real, D_fake)
            # Backpropagate the discriminator loss and take an optimizer step
            d_error.backward()
            D_solver.step()

            # Generator step
            # Zero gradients in the generator
            G_solver.zero_grad()
            # Generate a batch of fake images using the generator
            noise = sample_noise(batch_size, noise_size).reshape((batch_size, noise_size, 1, 1)).cuda()
